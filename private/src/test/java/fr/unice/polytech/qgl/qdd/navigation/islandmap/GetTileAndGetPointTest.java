package fr.unice.polytech.qgl.qdd.navigation.islandmap;
...
/**
 * Created by danial on 24/12/2015.
 * */
public class GetTileAndGetPointTest extends IslandMapTester {
    ...
    public void testSetCurrentPosition(){
        createXByYMap(10,10);
        setCurrentPositionMethod(1,1);
        List<String> creeks = new ArrayList<>();
        creeks.add("danial");
        getTileMethod(7,7).setGround();
        getTileMethod(7,7).addCreeks(creeks);

        Assert.assertEquals(map.x(), 1);

        map.setCurrentTile("danial");

        Assert.assertEquals(map.x(), 7);
        Assert.assertEquals(map.y(), 7);
    }
}
