...
public class IslandMap implements TileListener{
    ...
    private static final int MINIMUM_MAP_DIMENSION = 26;
    ...
    */
    ...
    public void initializeMapThroughEcho(Compass direction, int range) {
        ...
        if(height > MINIMUM_MAP_DIMENSION && width > MINIMUM_MAP_DIMENSION) {
            initializeMap();
        }
    }
    ...
    public void updateMapThroughEcho(boolean isGround, int range, Compass direction) {
        ...
        //update map with ground detected by echo
            ...
            switch (direction) {
                case NORTH:
                    tiles.addAll(updateMapThroughEcho(y()+groundDistance,true));
                    break;
                case EAST:
                    tiles.addAll(updateMapThroughEcho(x()+groundDistance,false));
                    break;
                case SOUTH:
                    tiles.addAll(updateMapThroughEcho(y()-groundDistance,true));
                    break;
    ...
    private Set<Tile> updateMapThroughEcho(Compass direction,int distance){
        Set<Tile> tiles = new HashSet<>();
        switch(direction) {
            case NORTH:
                tiles.addAll(getTiles(x() - 1, y() + 1, direction, distance));
    ...
    private Set<Tile> updateMapThroughEcho(int distance, boolean atXAxis){
        Set<Tile> tiles = new HashSet<>();
        if (atXAxis){
            tiles.add(getTile(x()-1, distance));
            tiles.add(getTile(x(), distance));
            tiles.add(getTile(x()+1, distance));
        }else{
            tiles.add(getTile(distance, y()-1));
            tiles.add(getTile(distance, y()));
            tiles.add(getTile(distance, y()+1));
        }
        return tiles;
    }
    ...
    public void updateMapThroughScan(List<Biome> biomes, boolean potentiallyExploitable) {
        Set<Tile> theScannedTiles = getSurroundingTiles(currentTile());
        theScannedTiles.add(currentTile());
        theScannedTiles.forEach(tile -> tile.addBiomes(biomes));

        if(potentiallyExploitable) {
            potentiallyExploitableTiles.addAll(theScannedTiles);
        }
    }
   ...
   public void updateMapThroughScout(Map<Resource,String> resources, int altitude,String direction,boolean exploitable){
        int x = x();
        int y = y();
        Compass compass = Compass.fromString(direction);
        Tile tile = getTile(x + compass.groundX(),y + compass.groundY());
        tile.addResources(resources);
        tile.setExplored();
        tile.setAltitude(altitude);
        if(exploitable)
            exploitableTiles.add(tile);
    }
    ...
    public void updatePotentiallyExploitableTiles(Set<Resource> resourcesToCollect) {
        potentiallyExploitableTiles.removeIf(tile -> tile == currentTile());
        exploitableTiles.removeIf(tile -> tile == currentTile());
        potentiallyExploitableTiles.removeIf(tile -> !tile.hasOrPotentiallyHasResourcesOfType(resourcesToCollect));
        exploitableTiles.removeIf(tile -> !tile.hasOrPotentiallyHasResourcesOfType(resourcesToCollect));
    }

    public void reUpdatePotentiallyExploitableTiles(Set<Resource> resourcesToCollect){
        for (Tile tile : groundTiles){
            if (tile.hasOrPotentiallyHasResourcesOfType(resourcesToCollect)){
                potentiallyExploitableTiles.add(tile);
            }
            if (tile.hasResourceOfType(resourcesToCollect)){
                exploitableTiles.add(tile);
            }
        }
    }
    ...
    public List<String> getCreeks() {
        List<String> theCreeks = new ArrayList<>();
        for (Tile tile: groundTiles){
            if (!tile.getCreeks().isEmpty()){
                theCreeks.addAll(tile.getCreeks());
            }
        }
        return theCreeks;
    }
    ...
    public void setCurrentTile(String creekId){
        for (Tile tile : groundTiles){
            if (!tile.getCreeks().isEmpty()){
                for (String s: tile.getCreeks()){
                    if (s.equals(creekId)){
                        currentPosition.x = getX(tile);
                        currentPosition.y = getY(tile);
                    }
                }
            }
        }
    ...
    public List<String> getCreeksWithPotentiallyExploitableRessources(){
        for(Tile  t : potentiallyExploitableTiles){
            if (!t.getCreeks().isEmpty())
            {
              return t.getCreeks();
            }
        }
    ...
    */
    ...
    Tile getTile(Tile referenceTile, Compass direction, int range) {
        ...
        return getTile(reference.x + (direction.groundX()*range), reference.y + (direction.groundY()*range));
    }
    ...
    int distanceToTile(Tile source, Tile destination){
        return Math.abs(xDiff(source,destination)) + Math.abs(yDiff(source,destination));
    }
    ...
    int yDiff(Tile source, Tile destination){
        if  (source == null || destination == null)
        {
           return height();
        }
        return getY(destination) - getY(source);
    }
    
    int xDiff(Tile source, Tile destination) {
        if (source == null || destination == null)
        {
           return width();
        }
        return getX(destination) - getX(source);
    }
    ...
    Set<Tile> getScannedTiles(){
        return scannedTiles;
    }
    
    Set<Tile> getUnknownTiles(){
        return unknownTiles;
    }

    public Set<Tile> getPotentiallyExploitableTiles() {
        return potentiallyExploitableTiles;
    }

    public Set<Tile> getExploitableTiles(){return exploitableTiles;}

    Compass direction(Tile reference, Tile destination) {
        int xref = tilesToPoints.get(reference).x;
        int xdest = tilesToPoints.get(destination).x;
        int yref = tilesToPoints.get(reference).y;
        int ydest = tilesToPoints.get(destination).y;
